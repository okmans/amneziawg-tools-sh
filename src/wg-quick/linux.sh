...
parse_options() {
	interface_section=0
	CONFIG_FILE="$1"
	echo "$CONFIG_FILE" | grep -E -q "^[a-zA-Z0-9_=+.-]{1,15}$" && CONFIG_FILE="/etc/amnezia/amneziawg/$CONFIG_FILE.conf"
...
add_if() {
	if ! cmd ip link add "$INTERFACE" type amneziawg; then
		...
		[ -z "$wg_impl" ] && wg_impl="amneziawg-go"
		[ -e "/sys/module/amneziawg" ] || ! command -v "$wg_impl" >/dev/null && exit $ret
...
del_if() {
	...
	if [ -z "$TABLE" ] || [ "$TABLE" = auto ] && table="$(get_fwmark)" && \
		awg show "$INTERFACE" allowed-ips | grep -E -q "/0(\ |$'\n'|$)" ; then
		for proto in "-4" "-6"; do
...
set_mtu_up() {
	...
	for endpoint in $(awg show "$INTERFACE" endpoints) ; do
	...
	if command -v iptables >/dev/null 2>&1; then
		for iptables in iptables ip6tables; do
			...
			while read -r line; do
				case "$line" in
					"COMMIT" | "-A "*"-m comment --comment \"awg-quick(8) rule for $INTERFACE\""* )
...
add_default() {
	...
	marker="-m comment --comment \"awg-quick(8) rule for $INTERFACE\""
	restore="*raw${N}"
...
cmd_down() {
	awg show interfaces | grep "$INTERFACE" || die "\`$INTERFACE' is not a WireGuard interface"
